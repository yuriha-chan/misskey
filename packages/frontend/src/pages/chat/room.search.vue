	...
	<MkFoldableSection v-if="searched">
		...
		<div v-if="searchResults.length > 0" class="_gaps_s">
			<div v-for="message in searchResults" :key="message.id" :class="$style.searchResultItem">
				<XMessage :item="{ type: 'message', data: message }" :membership="{ user: message.fromUser, id: '', createdAt: '', userId: '', roomId: '', hasLeft: false } as Misskey.entities.ChatRoomMembership" :isSearchResult="true"/>
			</div>
		</div>
