		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.rateLimitFactor, 'rateLimitFactor'])">
				...
				<div class="_gaps">
					...
					<MkRange :modelValue="role.policies.rateLimitFactor.value * 100" :min="0" :max="500" :step="2" :textConverter="(v) => `${v}%`" @update:modelValue="v => role.policies.rateLimitFactor.value = (v / 100)">
						<template #label>{{ i18n.ts._role._options.rateLimitFactor }}</template>
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canMentionMany, 'canMentionMany'])">
				...
				<template #suffix>
					<span v-if="role.policies.canMentionMany.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canMentionMany.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canMentionMany)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canMentionMany.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canMentionMany.value" :disabled="role.policies.canMentionMany.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canMentionMany.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
