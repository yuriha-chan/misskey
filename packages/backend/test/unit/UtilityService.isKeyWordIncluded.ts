...
describe('parseFilter', () => {
	...
	test('empty string falls back to ["or"] (parse failure)', () => {
		// The parser throws on empty input → isKeyWordIncluded wraps to ["or"]
...
describe('isKeyWordIncluded', () => {
	...
	test('empty keyWords → false', () => {
		expect(svc.isKeyWordIncluded([], 'text', '', '', [])).toBe(false);
	});

	test('empty text + cw + files → false', () => {
		expect(svc.isKeyWordIncluded(['hello'], '', '', '', [])).toBe(false);
	});

	test('empty text but has files + hasFile filter → true', () => {
		expect(svc.isKeyWordIncluded(['(hasFile)'], '', '', '', ['file1'])).toBe(true);
	});

	// ── keyword ──

	test('keyword matches substring', () => {
		expect(svc.isKeyWordIncluded(['hello'], 'hello world', '', '', [])).toBe(true);
	});

	test('keyword does not match', () => {
		expect(svc.isKeyWordIncluded(['xyz'], 'hello world', '', '', [])).toBe(false);
	});

	test('multiple keyWords: any match', () => {
		expect(svc.isKeyWordIncluded(['alpha', 'beta'], 'beta test', '', '', [])).toBe(true);
	});

	test('multiple keyWords: none match', () => {
		expect(svc.isKeyWordIncluded(['alpha', 'beta'], 'gamma test', '', '', [])).toBe(false);
	});

	// ── regexp ──

	test('regexp matches', () => {
		expect(svc.isKeyWordIncluded(['/foo/i'], 'hello FOO world', '', '', [])).toBe(true);
	});

	test('regexp does not match', () => {
		expect(svc.isKeyWordIncluded(['/foo/'], 'hello bar world', '', '', [])).toBe(false);
	});

	test('regexp with bad pattern → false (not throw)', () => {
		expect(svc.isKeyWordIncluded(['/[invalid/'], 'text', '', '', [])).toBe(false);
	});

	// ── slow regexp ──

	test('slowRegexp matches', () => {
		expect(svc.isKeyWordIncluded(['slow/foo/i'], 'hello FOO world', '', '', [])).toBe(true);
	});

	test('slowRegexp does not match', () => {
		expect(svc.isKeyWordIncluded(['slow/bar/'], 'hello foo world', '', '', [])).toBe(false);
	});

	test('slowRegexp with bad pattern → false (not throw)', () => {
		expect(svc.isKeyWordIncluded(['slow/[invalid/'], 'text', '', '', [])).toBe(false);
	});

	// ── and / or ──

	test('and: all match → true', () => {
		expect(svc.isKeyWordIncluded(['(and hello world)'], 'hello world', '', '', [])).toBe(true);
	});

	test('and: one fails → false', () => {
		expect(svc.isKeyWordIncluded(['(and hello xyz)'], 'hello world', '', '', [])).toBe(false);
	});

	test('or: any match → true', () => {
		expect(svc.isKeyWordIncluded(['(or xyz hello)'], 'hello world', '', '', [])).toBe(true);
	});

	test('or: none match → false', () => {
		expect(svc.isKeyWordIncluded(['(or xyz abc)'], 'hello world', '', '', [])).toBe(false);
	});

	// ── not ──

	test('not negates match', () => {
		expect(svc.isKeyWordIncluded(['(not hello)'], 'hello world', '', '', [])).toBe(false);
	});

	test('not negates non-match', () => {
		expect(svc.isKeyWordIncluded(['(not xyz)'], 'hello world', '', '', [])).toBe(true);
	});

	// ── shorterThan / longerThan ──

	test('shorterThan: below threshold → true', () => {
		expect(svc.isKeyWordIncluded(['(shorterThan 10)'], 'short', '', '', [])).toBe(true);
	});

	test('shorterThan: at threshold → false', () => {
		expect(svc.isKeyWordIncluded(['(shorterThan 5)'], 'hello', '', '', [])).toBe(false);
	});

	test('shorterThan: above threshold → false', () => {
		expect(svc.isKeyWordIncluded(['(shorterThan 3)'], 'hello', '', '', [])).toBe(false);
	});

	test('longerThan: above threshold → true', () => {
		expect(svc.isKeyWordIncluded(['(longerThan 3)'], 'hello', '', '', [])).toBe(true);
	});

	test('longerThan: at threshold → false', () => {
		expect(svc.isKeyWordIncluded(['(longerThan 5)'], 'hello', '', '', [])).toBe(false);
	});

	test('longerThan: below threshold → false', () => {
		expect(svc.isKeyWordIncluded(['(longerThan 10)'], 'short', '', '', [])).toBe(false);
	});
	...
	test('hasFile without files → false', () => {
		expect(svc.isKeyWordIncluded(['(hasFile)'], 'text', '', '', [])).toBe(false);
	});
	...
	test('cw scoping: match in cw', () => {
		expect(svc.isKeyWordIncluded(['(cw secret)'], 'text', 'this is secret content', '', [])).toBe(true);
	});

	test('cw scoping: no match in cw (only in text)', () => {
		expect(svc.isKeyWordIncluded(['(cw secret)'], 'this is secret content', 'clean', '', [])).toBe(false);
	});

	// ── text scoping ──

	test('text scoping: match in text', () => {
		expect(svc.isKeyWordIncluded(['(text body)'], 'body text', 'header text', '', [])).toBe(true);
	});

	test('text scoping: no match in text', () => {
		expect(svc.isKeyWordIncluded(['(text body)'], 'clean', 'body text', '', [])).toBe(false);
	});
	...
	test('cw non-empty: delegates testText to cw', () => {
		// When cw is non-empty, the isKeyWordIncluded passes cw as testText,
		// so textAndChoices is ignored even if it would match.
		expect(svc.isKeyWordIncluded(['alert'], 'alert text', 'clean CW', '', [])).toBe(false);
	});

	test('cw non-empty: matches in cw', () => {
		expect(svc.isKeyWordIncluded(['alert'], 'clean text', 'alert CW', '', [])).toBe(true);
	});

	// ── invalid filter fallback ──

	test('invalid filter string → ["or"] fallback → never matches', () => {
		expect(svc.isKeyWordIncluded([')'], 'any text', '', '', [])).toBe(false);
	});

	// ── space-separated AND keyword behavior ──

	test('space-separated barewords: AND semantics', () => {
		expect(svc.isKeyWordIncluded(['hello world'], 'hello beautiful world', '', '', [])).toBe(true);
	});

	test('space-separated barewords: AND fail when one missing', () => {
		expect(svc.isKeyWordIncluded(['hello xyz'], 'hello world', '', '', [])).toBe(false);
	});

	// ── quoted multi-word: literal substring (not space-separated AND) ──

	test('quoted multi-word: literal substring match', () => {
		expect(svc.isKeyWordIncluded(['"hello world"'], 'this is hello world today', '', '', [])).toBe(true);
	});

	test('quoted multi-word: no match as separate words', () => {
		expect(svc.isKeyWordIncluded(['"hello world"'], 'hello beautiful world', '', '', [])).toBe(false);
	});

	// ── -prefixed bareword behavior ──

	test('-prefixed bareword matches as keyword', () => {
		expect(svc.isKeyWordIncluded(['-foo'], 'this contains -foo yes', '', '', [])).toBe(true);
	});

	// ── not + cw ──

	test('not with cw', () => {
		expect(svc.isKeyWordIncluded(['(not (cw bad))'], 'text', 'good', '', [])).toBe(true);
		expect(svc.isKeyWordIncluded(['(not (cw bad))'], 'text', 'bad', '', [])).toBe(false);
	});
});

// ── TDD: poll operator precedence bug (Task 2) ──

describe('poll operator (TDD)', () => {
	const svc = makeService();

	test('poll: 2+ matches with threshold 2 → BUG: currently fails', () => {
